//每个线程同时加密64块即1024B数据
//将数据预处理，事先翻转
//在远端服务器块内线程256：21.483 Gbps
//远端服务器块内线程512：22.75 Gbps
...
class trans_aes_block
{
public:
    word_t block[BLOCK_SIZE];
};
...
__global__ void AES_Encrypt(trans_aes_block trans_aes_block_array[],word_t (* rk)[BLOCK_SIZE],int block_number){
    int global_thread_index = blockDim.x*blockIdx.x + threadIdx.x;
    int stride=blockDim.x*gridDim.x;
    __shared__ word_t key[11][BLOCK_SIZE]; 

    for(int real_thread=global_thread_index;real_thread < block_number;real_thread+=stride){
        if(threadIdx.x==0){
            memcpy(key,rk,11*BLOCK_SIZE*sizeof(word_t));
        }
        word_t block[BLOCK_SIZE];
        memcpy(block,trans_aes_block_array[real_thread].block,BLOCK_LEN);

        int round;
        bs_addroundkey(block,key[0]);
        for (round = 1; round < 10; round++)
        {
            bs_apply_sbox(block);
            bs_shiftmix(block);
            bs_addroundkey(block,key[round]);
        }
        bs_apply_sbox(block);
        bs_shiftrows(block);
        bs_addroundkey(block,key[10]);

        memcpy(trans_aes_block_array[real_thread].block,block,BLOCK_LEN);
...
int main(int argc, char* argv[]){
    ...
    trans_aes_block* trans_aes_block_array;
    ...
    if(number_of_zero_pending != 0){
        ...
        trans_aes_block_array=new trans_aes_block[block_number+1];
    }
    else{
        aes_block_array = new aes_block[ block_number ];
        trans_aes_block_array=new trans_aes_block[block_number];
    }
    ...
    word_t block[BLOCK_SIZE];
    for(int i=0;i<block_number;i++){
        memcpy(block,&aes_block_array[i],BLOCK_LEN);
        bs_transpose(block);
        memcpy(&trans_aes_block_array[i],block,BLOCK_LEN);
    }

    printf("数据预处理结束\n");

    word_t block2[BLOCK_SIZE];
    for(int i=0;i<block_number;i++){
    memcpy(block2,trans_aes_block_array[i].block,BLOCK_LEN);
    bs_transpose_rev(block2);
    memcpy(aes_block_array[i].block,block2,BLOCK_LEN);
    f1printBytes(aes_block_array[i].block, BLOCK_LEN, en_fp);
    }

    // trans_aes_block *cuda_trans_aes_block_array;
    ...
    // cudaMalloc(&cuda_trans_aes_block_array, block_number*sizeof(class trans_aes_block));
    // cudaMalloc(&cuda_key,11*BLOCK_SIZE*sizeof(word_t));
    // cudaMemcpy(cuda_trans_aes_block_array, trans_aes_block_array, block_number*sizeof(class trans_aes_block), cudaMemcpyHostToDevice);
    ...
    // AES_Encrypt <<<BlockperGrid,ThreadperBlock>>>(cuda_trans_aes_block_array,cuda_key,block_number);
    ...
    // cudaMemcpy(trans_aes_block_array, cuda_trans_aes_block_array, block_number*sizeof(class aes_block), cudaMemcpyDeviceToHost);
